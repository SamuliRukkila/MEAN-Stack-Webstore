...
const UserSchema = new mongoose.Schema({
  firstname: { type: String, required: true, match: /[-a-öA-Ö]+/ },
  surname: { type: String, required: true, match: /[-a-öA-Ö]+/ },
  email: { type: String, required: true, unique: true, match: /^([a-zA-Z0-9])(([\-.]|[_]+)?([a-zA-Z0-9]+))*(@){1}[a-z0-9]+[.]{1}(([a-z]{2,3})|([a-z]{2,3}[.]{1}[a-z]{2,3}))/ },
  address: { type: String, required: true, match: /[-a-öA-Ö0-9 ]+/ },
  postnumber: { type: Number, maxlength: 5,required: true, match: /[0-9]{5}$/ },
  city: { type: String, required: true, match: /[-a-öA-Ö]+/ },
